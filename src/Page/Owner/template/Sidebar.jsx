...
function Sidebar() {
    ...
    return (
        <nav className="w-full h-full flex flex-col justify-center items-center text-gray-500 bg-gray-200 px-3">
            <header className="hidden sm:flex h-10 xs:h-24 w-full md:w-5/6 justify-center items-center">
                <Logo.LogoBlack />
            </header>
            <section className="pl-0 md:pl-3 h-4/5 sm:h-5/6 w-full flex sm:flex-col justify-around md:justify-evenly lg:justify-around">
                ...
                <ul className="w-full sm:w-full h-full flex justify-between sm:justify-start sm:flex-col gap-2 sm:mt-5">
                    {
                        navigateData.map((data) => (
                            <li key={data.id} className="w-full h-12">
                                <SidebarMenu
                                    link={data.link}
                                    icon={data.icon}
                                    text={data.text}
